...
import { useCompleteSendMoney, useVerifySendMoney } from './api/route';
...
const SendMoney = () => {
    ...
    const onSubmit: SubmitHandler<Inputs> = async (sendMoney) => {
        ...
        sendMoney.method = 'sendMoney';
        sendMoney.sender_name = user?.name;
        sendMoney.sender_phone_number = user?.phone_number;

        // Verified send money method
        try {
            const res = await verifySendMoney.mutateAsync(sendMoney);
            console.log(res);
    ...
    // Handle button animation and complete send money
                ...
                <Dialog open={true} onOpenChange={() => setIsVerified(null)}>
                    <DialogContent className="md:max-w-xl w-full bg-popover-foreground text-white p-6 rounded-lg shadow-lg">
                        <DialogHeader className="text-center">
                            <DialogTitle className="text-2xl font-bold text-emerald-500">Send money Summary</DialogTitle>
